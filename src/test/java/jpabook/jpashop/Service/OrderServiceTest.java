...
public class OrderServiceTest {
    ...
    public void 상품주문() throws Exception  {
        //given
        Member member = createMember(); //아래것을 메소드로 만든것
    ...
    @Test(expected = NotEnoughStockExcpetion.class) //예외가 생길시 테스트가 통과됨 //테스트가 정상이면 오류뜸
    public void 상품주문_재고수량초과() throws Exception  {
        //given
        Member member = createMember();
        Item book = createBook("시골 JPA", 10000, 10);

        int orderCount = 11; //초과수량
    ...
    public void 주문취소() throws Exception  {
        ...
        assertEquals("주문취소된 상품은 그만큼 재고가 늘어나야한다", 10, book.getStockQuantity());
    }
    ...
    private Book createBook(String name, int price, int stockQuantity) {
        Book book = new Book();
        book.setName(name); //ctrl + alt + p 로 옵션 꺼내기
        book.setPrice(price);
